...
public class SearchFlightsOverviewController{
     ...
     */
    @FXML TextField        numberField;                      //text field, pattern string for searching by flight number
    @FXML TextField        planeID;                          //text field, pattern string for searching by plain id
    @FXML TextField        flightTimeToTextField;
    //text field, converting to date for searching by travel time (less then this)
    @FXML TextField        flightTimeFromTextField;
    //text field, converting to date for searching by travel time (more then this)
    @FXML TextField        departureDateFromTextField;       //text field, add time component to departureFrom date
    @FXML TextField        departureDateToTextField;         //text field, add time component to departureTo date
    @FXML TextField        arrivingDateFromTextField;        //text field, add time component to arrivingFrom date
    @FXML TextField        arrivingDateToTextField;          //text field, add time component to arrivingTo date
    @FXML DatePicker       arrivingDateFromDatePicker;
    //DatePicker, date for searching by arriving date (more then this or exact if arrivingTo==null)
    @FXML DatePicker       arrivingDateToDatePicker;
    //DatePicker, date for searching by arriving date (less then this)
    @FXML DatePicker       departureDateFromDatePicker;
    //DatePicker, date for searching by departure date (more then this or exact if departureTo==null)
    @FXML DatePicker       departureDateToDatePicker;//DatePicker, date for searching by departure date (less then this)
    @FXML ChoiceBox<Route> routeBox;                  //ChoiceBox with routes, existing in DataModel, search by route
    @FXML Button           searchSearchFlightsButton;           //Search button
    @FXML CheckBox         flightTimeCheckBox;                //CheckBox, if checked - search will use travelDate
    ...
    public void handleSearchAction( ActionEvent event ){
        ...
        if( departureDateFromDatePicker.getValue() != null ){
            try{
                dfDate = format.parse( departureDateFromDatePicker.getEditor().getText() + " " +
                                       departureDateFromTextField.getText() );
        ...
        if( departureDateToDatePicker.getValue() != null ){
            try{
                dtDate = format.parse(
                        departureDateToDatePicker.getEditor().getText() + " " + departureDateToTextField.getText() );
            }catch( ParseException e ){
        ...
        if( arrivingDateFromDatePicker.getValue() != null ){
            try{
                afDate = format.parse(
                        arrivingDateFromDatePicker.getEditor().getText() + " " + arrivingDateFromTextField.getText() );
            }catch( ParseException e ){
        ...
        if( arrivingDateToDatePicker.getValue() != null ){
            try{
                atDate = format.parse(
                        arrivingDateToDatePicker.getEditor().getText() + " " + arrivingDateToTextField.getText() );
            }catch( ParseException e ){
        ...
        try{
            tfDate = travelFormat.parse( flightTimeFromTextField.getText() ).getTime();
        }catch( ParseException e ){
        ...
        try{
            ttDate = travelFormat.parse( flightTimeToTextField.getText() ).getTime();
        }catch( ParseException e ){

        }
        try{
            controller.setFlights( FXCollections.observableArrayList( Main.getEngine()
                                                                          .searchFlight( numberField.getText() ,
                                                                                         routeBox.getValue() , "" , "" ,
                                                                                         planeID.getText() , dfDate ,
                                                                                         dtDate , afDate , atDate ,
                                                                                         flightTimeCheckBox
                                                                                                 .isSelected() ?
                                                                                         tfDate : null ,
                                                                                         flightTimeCheckBox
